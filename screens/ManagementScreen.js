...
export default function ManagementScreen({navigation}) {
    ...
    useFocusEffect(
      ...
      fetch('https://transide-backend.vercel.app/users/usersByOffice', {
        method: 'PUT',
    ...
    const selectByDefault = (index) => {
      ...
      fetch('https://transide-backend.vercel.app/users/newOfficeByDefault', {
        method: 'PUT',
        headers: {'Content-Type' : 'application/json'},
        body: JSON.stringify({
          token:  user.token ,
 ...
 const addNewOffice = () => {
  ...
  if(isAllReadyExist.length>0){
    ...
    fetch('https://transide-backend.vercel.app/users/newOffice', {
        method: 'POST',
        headers: {'Content-Type' : 'application/json'},
        body: JSON.stringify({
